...
import blockSender from "../../assets/content/block-sender.png";
...
Emails often contain tracking pixels that tell the sender when and where you opened their emails. We find that creepy! Firefox Relay removes trackers it recognizes. Shroud.email dynamically blocks tracking pixels, including ones it hasn't seen before. And to be on the safe side, we route all images through our own servers so that senders never learn your IP address.
...
<Image
  src={blockSender}
  alt="Web UI showing how to block a sender in Shroud.email"
  class="max-w-full sm:max-w-[572px] mx-auto"
  layout="constrained"
  width={572}
  loading="lazy"
  decoding="async"
...
Both services offer tracker protection, but Shroud.email also blocks tracking pixels it hasn't seen before and hides your IP address when images load.
